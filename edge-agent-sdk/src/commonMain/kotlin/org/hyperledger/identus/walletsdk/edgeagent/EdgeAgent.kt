...
class EdgeAgent {
     ...
     * @param logger An optional EdgeLogger instance used by the EdgeAgent if provided, otherwise a EdgeLoggerImpl with
     *               LogComponent.PRISM_AGENT will be used.
     * @param agentOptions Options to configure certain features with in the edge agent.
     */
    @JvmOverloads
    constructor(
        apollo: Apollo,
     ...
     * @param logger The logger for EdgemAgent. Default is EdgeLoggerImpl with LogComponent.PRISM_AGENT.
     * @param agentOptions Options to configure certain features with in the Edge Agent.
     */
    @JvmOverloads
    constructor(
    ...
    suspend fun prepareRequestCredentialWithIssuer(
        ...
        return when (val type = pollux.extractCredentialFormatFromMessage(offer.attachments)) {
            ...
            CredentialType.ANONCREDS_OFFER -> {
                ...
                // TODO: Create new edge agent error message
                throw org.hyperledger.identus.walletsdk.domain.models.UnknownError.SomethingWentWrongError(
